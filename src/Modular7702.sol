...
import {IERC7579AccountConfig} from "oz-5.4/account/extensions/draft-AccountERC7579.sol";
import {EnumerableSet} from "oz-5.4/utils/structs/EnumerableSet.sol";
import {PackedUserOperation} from "oz-5.4/interfaces/draft-IERC4337.sol";
import {IERC1271} from "oz-5.4/interfaces/IERC1271.sol";
import {
    IERC7579Module,
    IERC7579Validator,
    IERC7579Execution,
...
} from "oz-5.4/interfaces/draft-IERC7579.sol";
import {ERC7579Utils, Mode, CallType, ExecType} from "oz-5.4/account/utils/draft-ERC7579Utils.sol";
import {EnumerableSet} from "oz-5.4/utils/structs/EnumerableSet.sol";
import {Bytes} from "oz-5.4/utils/Bytes.sol";
import {Packing} from "oz-5.4/utils/Packing.sol";
import {Calldata} from "oz-5.4/utils/Calldata.sol";
import {Account} from "oz-5.4/account/Account.sol";
import {ECDSA} from "oz-5.4/utils/cryptography/ECDSA.sol";
import {AbstractSigner} from "oz-5.4/utils/cryptography/signers/AbstractSigner.sol";

interface IModular7702 {
    struct Storage {
        EnumerableSet.AddressSet validators;
        EnumerableSet.AddressSet executors;
        mapping(bytes4 selector => address) fallbacks;
    }
...
/**
 * @dev A modular contract account designed for delegation by EOAs and compatible with EntryPoint v0.8.
 */
contract Modular7702 is
    IModular7702,
    Account,
    IERC1271,
    IERC7579Execution,
    ...
    IERC7579ModuleConfig
{
    ...
    /// @dev cast index-erc7201 ethaccount.Modular7702.v0.0.1
    bytes32 private constant STORAGE_SLOT = 0x2eb5d58dab6d6af02acdaa2735e528431acd7cea3e9f8f17b48f2b5e8bf87600;
    ...
    /**
     * @dev Check EOA signature validation first and then module based validation.
    ...
    function isValidSignature(bytes32 hash, bytes calldata signature) public view virtual returns (bytes4) {
        ...
        if (signature.length == 65 && _rawSignatureValidation(hash, signature)) {
            return this.isValidSignature.selector;
        }

        // module based validation, check signature length is enough for extraction
     ...
     * @dev Replicates the behavior of
     * https://github.com/eth-infinitism/account-abstraction/blob/v0.8.0/contracts/accounts/Simple7702Account.sol#L40
     */
        ...
        returns (bool)
    {
        ...
        if (moduleTypeId == MODULE_TYPE_VALIDATOR) return $.validators.contains(module);
        if (moduleTypeId == MODULE_TYPE_EXECUTOR) return $.executors.contains(module);
        if (moduleTypeId == MODULE_TYPE_FALLBACK) return $.fallbacks[bytes4(additionalContext[0:4])] == module;
        return false;
    }
    ...
    function _installModule(uint256 moduleTypeId, address module, bytes memory initData) internal virtual {
        ...
        if (moduleTypeId == MODULE_TYPE_VALIDATOR) {
            require($.validators.add(module), ERC7579Utils.ERC7579AlreadyInstalledModule(moduleTypeId, module));
        } else if (moduleTypeId == MODULE_TYPE_EXECUTOR) {
            require($.executors.add(module), ERC7579Utils.ERC7579AlreadyInstalledModule(moduleTypeId, module));
        } else if (moduleTypeId == MODULE_TYPE_FALLBACK) {
            bytes4 selector;
            (selector, initData) = _decodeFallbackData(initData);
            require(
                $.fallbacks[selector] == address(0), ERC7579Utils.ERC7579AlreadyInstalledModule(moduleTypeId, module)
            );
    ...
    function _uninstallModule(uint256 moduleTypeId, address module, bytes memory deInitData) internal virtual {
        ...
        if (moduleTypeId == MODULE_TYPE_VALIDATOR) {
            require($.validators.remove(module), ERC7579Utils.ERC7579UninstalledModule(moduleTypeId, module));
        } else if (moduleTypeId == MODULE_TYPE_EXECUTOR) {
            require($.executors.remove(module), ERC7579Utils.ERC7579UninstalledModule(moduleTypeId, module));
        } else if (moduleTypeId == MODULE_TYPE_FALLBACK) {
            bytes4 selector;
    ...
    function _fallback() internal virtual returns (bytes memory) {
        ...
        (bool success, bytes memory returndata) = handler.call{value: msg.value}(abi.encodePacked(msg.data, msg.sender));
